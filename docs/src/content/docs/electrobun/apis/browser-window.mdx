...
| `allowedProtocols` | `{ views?: boolean; appData?: boolean }` | `{ views: true, appData: false }` | Grant the initial webview access to Electrobun's filesystem-backed URL schemes. |
...
| `kiosk` | `boolean` | `false` | Start in restricted fullscreen kiosk mode. |
...
Kiosk mode is restricted fullscreen intended for unattended displays and
terminals. While it is active, Electrobun blocks the usual keyboard shortcuts
that leave fullscreen, hide, minimize, or close the window. On macOS it also
hides the Dock and menu bar and disables app switching, Force Quit, and session
termination UI. Call `setKiosk(false)` before attempting to leave fullscreen.
If the window was already fullscreen when kiosk mode began, it remains
fullscreen when kiosk mode ends.

Kiosk mode is an application-level control, not an operating-system security
boundary. For a locked public terminal, combine it with the platform's managed
kiosk or assigned-access facilities.
...
| `win.setFullScreen(b)` / `setKiosk(b)` / `setAlwaysOnTop(b)` / `setVisibleOnAllWorkspaces(b)` | `setWindowFullScreen` / `setWindowKiosk` etc. | `set_window_full_screen` / `set_window_kiosk` etc. | `SetWindowFullScreen` / `SetWindowKiosk` etc. |
